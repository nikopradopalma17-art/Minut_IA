...
impl AudioMixerRingBuffer {
    ...
    fn add_samples(&mut self, device_type: DeviceType, samples: Vec<f32>) {
        // Log buffer health periodically for diagnostics. Called from both
        // the mic and system capture threads, so the counter must be atomic —
        // a `static mut` increment here was a data race (UB).
        static SAMPLE_COUNTER: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(0);
        if SAMPLE_COUNTER.fetch_add(1, std::sync::atomic::Ordering::Relaxed) % 200 == 0 {
...
pub struct AudioCapture {
    ...
    // Retained from the constructor; the recording path now flows through
    // AudioPipeline::recording_sender_for_mixed instead.
    #[allow(dead_code)]
...
pub struct AudioPipeline {
    ...
    // Kept alongside the loop's stop-signal handling; the run loop
    // deliberately does NOT poll state.is_recording() (see run()).
    #[allow(dead_code)]
    state: Arc<RecordingState>,
    ...
    // Cumulative mixed-stream time; anchors per-window mic/system energy
    // reported to recording_commands' active-speaker timeline (Tier 1)
...
impl AudioPipeline {
    pub fn new(
        ...
        // A VAD init failure (ONNX runtime missing/quarantined, OOM) must be
        // a start_recording error the user sees, not a panic.
