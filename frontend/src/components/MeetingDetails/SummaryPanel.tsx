...
export function SummaryPanel({
  ...
  const effectiveLangLabel = summaryLang ? labelForCode(summaryLang) : t('summary.language_auto');
  const isLocalFallbackLanguage = summaryLangStorage === 'local_fallback';
  const autoSubtitle = isLocalFallbackLanguage
    ? t('summary.language_auto_subtitle_fallback')
    : t('summary.language_auto_subtitle_default');
  ...
  useEffect(() => {
    ...
    const loadSummaryLanguage = async () => {
      try {
        ...
        toast.warning(t('summary.language_load_failed'), {
          description: t('summary.language_load_failed_desc'),
        });
        ...
        try {
          ...
          if (
            ...
            if (saved.storage === 'local_fallback') {
              toast.info(t('summary.language_saved_device'), {
                description: t('summary.language_saved_device_desc'),
              });
            }
            if (request.language) {
          ...
          if (
            ...
            toast.error(t('summary.language_save_failed'));
    ...
    <Popover open={langPickerOpen} onOpenChange={setLangPickerOpen}>
      <PopoverTrigger asChild>
        <Button
          variant="outline"
          size="sm"
          title={`${t('summary.language_button_title').replace('{label}', effectiveLangLabel)}${isLocalFallbackLanguage ? t('summary.language_saved_device_suffix') : ''}`}
          aria-label={t('summary.language_button_aria')}
        >
          <Languages size={18} />
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex items-center justify-center flex-1">
            <div className="text-center">
              ...
              <p className="text-muted-foreground">{t('meeting_details.generating_summary')}</p>
            </div>
          </div>
        </div>
      ) : !aiSummary ? (
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          {summaryResponse && (
            <div className="fixed bottom-0 left-0 right-0 bg-card shadow-lg p-4 max-h-1/3 overflow-y-auto">
              <h3 className="font-heading text-lg font-semibold mb-2 text-foreground">{t('summary.meeting_summary')}</h3>
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-muted/40 border border-border p-4 rounded-lg">
                  <h4 className="font-medium mb-1 text-foreground">{t('summary.key_points')}</h4>
                  <ul className="list-disc pl-4">
                    {summaryResponse.summary.key_points.blocks.map((block, i) => (
                      <li key={i} className="text-sm text-muted-foreground">{block.content}</li>
                    ))}
                  </ul>
                </div>
                <div className="bg-muted/40 border border-border p-4 rounded-lg mt-4">
                  <h4 className="font-medium mb-1 text-foreground">{t('summary.action_items')}</h4>
                  <ul className="list-disc pl-4">
                ...
                <div className="bg-muted/40 border border-border p-4 rounded-lg mt-4">
                  <h4 className="font-medium mb-1 text-foreground">{t('summary.decisions')}</h4>
                  <ul className="list-disc pl-4">
                ...
                <div className="bg-muted/40 border border-border p-4 rounded-lg mt-4">
                  <h4 className="font-medium mb-1 text-foreground">{t('summary.main_topics')}</h4>
                  <ul className="list-disc pl-4">
              ...
              {summaryResponse.raw_summary ? (
                <div className="mt-4">
                  <h4 className="font-medium mb-1 text-foreground">{t('summary.full_summary')}</h4>
                  <p className="text-sm whitespace-pre-wrap text-muted-foreground">{summaryResponse.raw_summary}</p>
                </div>
              ) : null}
            </div>
          )}
          ...
          {summaryStatus !== 'idle' && (
            <div className={`mt-4 p-4 rounded-lg ${summaryStatus === 'error' ? 'bg-destructive/10 text-destructive' :
              summaryStatus === 'completed' ? 'bg-impulso-ocean/10 text-impulso-ocean' :
