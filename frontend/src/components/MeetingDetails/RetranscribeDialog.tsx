...
export function RetranscribeDialog({
      ...
      const unlistenComplete = await listen<RetranscriptionResult>(
        ...
        async (event) => {
          if (event.payload.meeting_id === meetingId) {
            ...
            setIsProcessing(false);
            toast.success(t('retranscribe.complete').replace('{n}', String(event.payload.segments_count)));
  ...
  const handleStartRetranscription = async () => {
    if (!meetingFolderPath) {
      setError(t('retranscribe.folder_missing'));
      return;
    }
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription>
            {isProcessing
              ? progress?.message || t('retranscribe.processing_desc')
              : error
                ? t('retranscribe.failed_desc')
            ...
            !isParakeetModel ? (
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium">{t('retranscribe.language')}</span>
                </div>
                <Select value={selectedLang} onValueChange={setSelectedLang}>
                  <SelectTrigger className="w-full">
                    <SelectValue placeholder={t('retranscribe.select_language')} />
                  </SelectTrigger>
              ...
              <div className="space-y-3">
                ...
                <p className="text-xs text-muted-foreground">
                  {t('retranscribe.parakeet_language_hint')}
                </p>
              </div>
            )
          )}
          ...
          {!isProcessing && !error && availableModels.length > 0 && (
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium">{t('retranscribe.model')}</span>
              </div>
              <Select value={selectedModelKey} onValueChange={setSelectedModelKey} disabled={loadingModels}>
                <SelectTrigger className="w-full">
                  <SelectValue placeholder={loadingModels ? t('retranscribe.loading_models') : t('retranscribe.select_model')} />
                </SelectTrigger>
                <SelectContent>
