...
export const RecordingStatusBar: React.FC<RecordingStatusBarProps> = ({ isPaused = false }) => {
  ...
  return (
    ...
    >
      <div className={`w-2 h-2 rounded-full ${isPaused ? 'bg-impulso-ocean' : 'bg-destructive animate-pulse'}`} />
      <span className={`text-sm ${isPaused ? 'text-impulso-ocean' : 'text-foreground'}`}>
