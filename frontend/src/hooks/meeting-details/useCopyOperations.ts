...
export function useCopyOperations({
  ...
  const fetchAllTranscripts = useCallback(async (meetingId: string): Promise<Transcript[]> => {
    try {
      ...
      toast.error(t('toasts.copy_fetch_failed'));
      return [];
    }
  }, []);
  ...
  const handleCopyTranscript = useCallback(async () => {
    ...
    if (!allTranscripts.length) {
      const error_msg = t('toasts.copy_no_transcripts');
    ...
    toast.success(t('toasts.transcript_copied'));
  ...
  const handleCopySummary = useCallback(async () => {
    try {
      ...
      if (!summaryMarkdown.trim()) {
        ...
        toast.error(t('toasts.copy_no_summary'));
        return;
      }
      ...
      toast.success(t('toasts.summary_copied'));
      ...
      toast.error(t('toasts.copy_summary_failed'));
    }
