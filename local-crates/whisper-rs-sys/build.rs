...
fn main() {
    ...
    // On Windows (MSVC), bindgen generates Linux-compatible bindings due to a known
    // issue with how libclang resolves system headers. Always use the pre-baked
    // Windows-compatible bindings on Windows to avoid this.
    let host = env::var("HOST").unwrap_or_default();
    let use_prebaked = env::var("WHISPER_DONT_GENERATE_BINDINGS").is_ok()
